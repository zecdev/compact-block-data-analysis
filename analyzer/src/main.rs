...
async fn main() -> Result<()> {
    ...
    if args.len() < 4 {
        ...
        eprintln!("  {} http://127.0.0.1:9067 http://127.0.0.1:8232 density 1000 density.csv", args[0]);
        eprintln!("  {} http://127.0.0.1:9067 http://127.0.0.1:8232 complete 2800000 2800100", args[0]);
        std::process::exit(1);
    }
    ...
    let (blocks, output_file) = match mode.as_str() {
        ...
        "density" => {
            if args.len() < 5 {
                eprintln!("Error: density mode requires <every_n>");
                std::process::exit(1);
            }
            let every_n: u64 = args[4].parse()?;
            let output = args.get(5).map(|s| s.as_str()).unwrap_or("density_sample.csv");
            let sampler = Sampler::new(
                SamplingStrategy::FixedDensity { every_n },
                current_tip,
                Some(42),
            );
